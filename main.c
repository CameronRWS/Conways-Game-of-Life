...
int main(int argc, char **argv) {
    ...
    if(quiescentState != -1) { //If quiscentState is not -1, then the 1DCA shouldn't be randomly initialized.
        ca = create1DCA(numCells, quiescentState); //Initialize the 1DCA to the quiescentState and allocate memory.
        ...
        for(int i = 0; i < ca->numCells; i++) { //For each cell in the 1DCA
    ...
    while(numSteps--) { //Simulate the 1DCA for the requested number of steps.
        stepCA(ca, &rule110, shouldWrapFlag); //Simulate one step of the 1DCA using rule 110.
        display1DCA(ca); //Display the current state of the 1DCA.
    }
    return 0;
}